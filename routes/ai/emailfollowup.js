...
app.post('/emailfollowup', async (req, res, next) => {
	try {
		...
		`${followUp ? `Opfølgning efter: Gratis prøveperiode ${followUp}\n` : ``}` + 
		...
		`${followUp ? `Opfølgning efter: Efterladt kurv${followUp}\n` : ``}` + 
		...
		`${productInfo ? `Produkt info: ervice som omhandler at lave social medie annoncer, såsom facebook/instagram, tiktok og snapchat annoncer.${productInfo}\n` : ``}` + 
		...
		const gptResponse = await openai.complete({
			...
			user: req.user._id,
			stream: false,
			stop: [`"""`, "Tone:", "Målgruppe:", "Opfølgning efter:", "Produkt navn:", "Produkt info:", "email:"],
		});
